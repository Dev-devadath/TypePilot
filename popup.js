...
document.addEventListener("DOMContentLoaded", function () {
  ...
  chrome.storage.local.get(["user", "isLoggedIn"], function (result) {
  ...
  chrome.storage.sync.get("commandKeyword", function (data) {
    if (data.commandKeyword) {
      ...
      chrome.storage.sync.set({ commandKeyword: defaultCommand + ":" });
    }
  ...
  if (modelSelect) {
    ...
    modelSelect.addEventListener("change", function () {
      ...
      // Save to storage
      chrome.storage.sync.set({ activeModel: modelName }, function () {
  ...
  function updateCommandKeyword(command) {
    ...
    chrome.storage.sync.set({ commandKeyword: command }, function () {
...
function updateCreditsDisplay() {
  ...
  resetFreeCreditsIfNeeded(() => {
    ...
    resetLoggedInFreeCreditsIfNeeded(() => {
      chrome.storage.local.get(["user", "freeCredits"], (data) => {
        let credits = 0;
...
function resetFreeCreditsIfNeeded(callback) {
  ...
  chrome.storage.local.get(["freeCreditsLastReset"], (data) => {
...
function resetLoggedInFreeCreditsIfNeeded(callback) {
  chrome.storage.local.get(["user", "userCreditsLastResetMonth"], (data) => {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (addCustomModelBtn && customModelModal) {
    addCustomModelBtn.addEventListener("click", function () {
      // Load existing custom model data if available
      chrome.storage.sync.get("customModel", function (data) {
        if (data.customModel) {
          customModelNameInput.value = data.customModel.name || "";
          customModelEndpointInput.value = data.customModel.endpoint || "";
          customModelKeyInput.value = data.customModel.key || "";
        } else {
          // Clear fields if no existing data
          customModelNameInput.value = "";
          customModelEndpointInput.value = "";
          customModelKeyInput.value = "";
        }
      });
  ...
  if (saveCustomModelBtn) {
    saveCustomModelBtn.addEventListener("click", () => {
      ...
      if (!name || !endpoint || !key) {
        alert("Please fill in all fields for the custom model.");
        return;
      }
      ...
      chrome.storage.sync.set(
        ...
        function () {
          ...
          if (modelSelect) {
            ...
            chrome.storage.sync.set({ activeModel: "custom" });
  ...
  chrome.storage.sync.get("customModel", (data) => {
